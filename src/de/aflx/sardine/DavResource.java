 ...
 *
 * @author jonstevens
 * @version $Id: DavResource.java 307 2011-11-09 10:24:53Z dkocher@sudo.ch $
 */
public class DavResource
{
	...
	/**
	 * Converts the given {@link Response} to a {@link de.aflx.sardine.DavResource}.
	 *
	 * @param response
	...
	{
		return getPropstat(response).getProp().getGetlastmodified();
		/*if (list.isEmpty()) {
			return null;
		}
	...
	private Propstat getPropstat(Response response){
	...
	private String getCreationDate(Response response)
	{
		...
		return getPropstat(response).getProp().getCreationdate();
        /*List<Propstat> list = response.getPropstat();
        if (list.isEmpty()) {
            return null;
        }
	...
	private String getContentType(Response response)
	{
		...
		String type = getPropstat(response).getProp().getGetcontenttype();
		
		// If there is no content type set in the properties, we will parse the href
		if (null == type) {
			if (this.getHref().toString().endsWith("/")) {
				type = HTTPD_UNIX_DIRECTORY_CONTENT_TYPE;
			} else {
	...
	/** modif by arns: to let sardine android to get size
	...
	{
		Propstat list = getPropstat(response);
		if(list.equals("") || null == list) {
			return -1L;
		}
		
		if (list.getProp().getGetcontentlength()!=null && !"".equals(list.getProp().getGetcontentlength())){
			return Long.parseLong(list.getProp().getGetcontentlength());
		}
	...
	{
		//add "|| (href.toString().endsWith("/") && contentLength==0)" for mediencenter.t-online.de which use another way to use a folder
		return HTTPD_UNIX_DIRECTORY_CONTENT_TYPE.equals(this.contentType) || (href.toString().endsWith("/") && contentLength==0);
	}
