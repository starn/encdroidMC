...
public class EDApplication extends Application {
	...
	// Volume list
	public static List<EDVolume> volumeList;
	...
	static {
		...
		try {
			...
			Logger.log("use native encryption api");
			//test for the user who have a bug of corrupted files
			...
			//t.printStackTrace();
			mNativePBKDF2ProviderAvailable = false;
			Logger.log("use java encryption api");
    ...
    BroadcastReceiver screenOffReceiver = new BroadcastReceiver() {    
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (intent.getAction().equals(Intent.ACTION_SCREEN_ON)) {
                ...
                PinCodeActivity.screenHasBeenOff=true;
                
        		//remove temp files if not deleted
        		File encDroidDir = new File(Environment.getExternalStorageDirectory(), EDVolumeBrowserActivity.ENCDROID_SD_DIR_NAME);
        		File[] files = encDroidDir.listFiles();
        		for (File f: files){
        			new SecureDeleteThread(f).start();
        		}
            }
