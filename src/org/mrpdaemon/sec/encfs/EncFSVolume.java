...
public class EncFSVolume {
	...
	public final static String[] CONFIG_FILE_NAMES = { ".encfs6.xml", "encfs6.xml", "Keyfile.bcx" };	
	...
	public EncFSFile getFile(String filePath) throws EncFSCorruptDataException,
		...
		String encryptedPath = new EncFSCrypto()
	 ...
	 *             File provider returned I/O error
	 */
	public boolean exists(String path) throws EncFSCorruptDataException, IOException {return pathExists(path);}
	public boolean pathExists(String path) throws EncFSCorruptDataException, IOException {
		validateAbsoluteFileName(path, "fileName");
		String encryptedPath = new EncFSCrypto().encodePath(this, path, ROOT_PATH);
		return fileProvider.exists(encryptedPath);
	}
	
	public boolean isDir(String path) throws EncFSCorruptDataException,
	IOException {
		validateAbsoluteFileName(path, "fileName");
		String encryptedPath = new EncFSCrypto().encodePath(this, path, ROOT_PATH);
		return fileProvider.isDirectory(encryptedPath);
	}
	...
	public static boolean isEncFSVolume(EncFSFileProvider fileProvider)
			throws IOException {
		for (String confFile:  EncFSVolume.CONFIG_FILE_NAMES){
			if (fileProvider.exists("/" + confFile)) return true;
		}
		return false;
		//return (fileProvider.exists("/" + EncFSVolume.CONFIG_FILE_NAME));
	}
	...
	public boolean makeDir(String dirPath) throws EncFSCorruptDataException,
		...
		String encryptedPath = new EncFSCrypto().encodePath(this, dirPath, ROOT_PATH);
	...
	private boolean copyOrMovePath(String srcPath, String dstPath,
		...
		String encSrcPath = new EncFSCrypto().encodePath(this, srcPath, ROOT_PATH);
		String encDstPath = new EncFSCrypto().encodePath(this, dstPath, ROOT_PATH);
	...
	public EncFSInputStream openInputStreamForPath(String filePath)
			...
			IOException {
		return getFile(filePath).openInputStream(0);
	}
	...
	public static boolean isConfigFile(String filename){
		for (String confFile:  EncFSVolume.CONFIG_FILE_NAMES){
			if (filename.equals(confFile)) return true;
		}
		return false;
	}
}
