...
class VolumeKey {
	...
	private static byte[] encryptVolumeKey(EncFSConfig config,
		...
		Key passKey = new EncFSCrypto().newKey(passKeyData);
		...
		byte[] cipherVolKeyData = new EncFSCrypto().encryptKeyData(volKeyData,
	...
	private static byte[] decryptVolumeKeyData(byte[] encryptedVolKey,
		...
		try {
			clearVolKeyData = new StreamCrypto().streamDecrypt(
					new StreamCrypto().newStreamCipher(), mac, passKey, passIvData,
